...
namespace MementoFX.Persistence.SqlServer.Helpers
{
    internal static class TableHelper
    {
        ...
        public static bool TableHasSchemaName(string tableName)
        {
        ...
        public static string GetSchemaName(string tableName)
        {
            ...
            if (parts.Length < 2)
            {
                throw new ArgumentException("Table name does not contain the schema on it.");
            }
            return parts[0];
        }
        public static string GetTableName(string tableName)
        {
            if (TableHasSchemaName(tableName))
            {
                var parts = tableName.Split('.');
                return parts[1];
        ...
        public static bool CheckIfTableExists(this SqlConnection connection, string tableName)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                if (TableHasSchemaName(tableName))
                {
                    command.CommandText = Commands.SelectTableAndSchemaExistsFormat;
                    command.Parameters.AddWithValue(Commands.SchemaNameParameterName, GetSchemaName(tableName));
                }
                else
                {
                    command.CommandText = Commands.SelectTableExistsFormat;
                }
                command.Parameters.AddWithValue(Commands.TableNameParameterName, GetTableName(tableName));
        ...
        private static void CreateIndex(this SqlConnection connection, string tableName, string columnName)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                var tableNamePart = GetTableName(tableName);
                //var indexName = string.Format(Commands.CreateIndexNameFormat, tableName, columnName);
                var indexName = string.Format(Commands.CreateIndexNameFormat, tableNamePart, columnName);
